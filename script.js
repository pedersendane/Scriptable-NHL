...
async function fetchHockeyGamesToday(teams) {
 ...
 data.gameWeek.forEach(gameWeek => {
     if(gameWeek.date == currentDateString){
         ...
         games.forEach(game => {
         console.log(game.homeTeam.commonName.default)
             let gameInfo = {
...
function getTeamDetails(team, teams) {
 ...
 // if its an nhl team, it current team will be set
 if(currentTeam){
  ...
  //If it is not a team object, like during the 4 nations face off
 else{

   let teamColor = getTeamColor(team)
   teamData = {
       fullName: team.commonName.default,
       triCode: team.abbrev,
       city: team.placeName.default,
       logo: team.logo,
       teamColor: teamColor,
...
function getTeamColor(currentTeam) {
 ...
 let teamName = currentTeam.fullName ? currentTeam.fullName : currentTeam.commonName.default;
 switch (teamName) {
     case "Anaheim Ducks":
         teamColor = "#F47A38";
         break;
